...
Good starting points are listed in `docs/CONTRIBUTOR_TASKS.md`.

Open an RFC before changing privacy claims, utility claims, policy schema, report schema, external model adapters, audit semantics, mapping semantics, restoration semantics, or benchmark methodology. See `docs/RFC_PROCESS.md`.
...
适合新贡献者的任务见 `docs/CONTRIBUTOR_TASKS.md`。

改变隐私主张、效用主张、策略 schema、报告 schema、外部模型 adapter、审计语义、映射语义、复原语义或 benchmark 方法前，应先提交 RFC。见 `docs/RFC_PROCESS.md`。
